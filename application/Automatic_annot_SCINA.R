...
Dimplot_with_ggplot <- function(object, group, colors = NULL){
  coord_Umap <- Embeddings(object[["umap"]])[,1:2]
  Proba_plot <- data.frame(coord_Umap,object[[group]])
  ggplot(Proba_plot, aes(UMAP_1,UMAP_2, color = Proba_plot[[group]]))+geom_point()+theme_cowplot()+scale_color_manual(name = group ,values = c(hue_pal()(length(levels(factor(Proba_plot[[group]]))))))
}

FeaturePlot_proba <- function(object, proba_col,slotNameAnnot, colors = NULL, BoolColScales = FALSE){
  validate(
  ...
  if(proba_col == "All_probabilities"){
    ...
    ggplot(Proba_plot, aes(UMAP_1,UMAP_2, color = Proba_plot[[slotNameAnnot]]))+geom_point(aes(alpha = proba))+theme_cowplot()+scale_color_manual(name = slotNameAnnot, values = c(hue_pal()(length(levels(factor(Proba_plot[[slotNameAnnot]])))-1),"lightgrey"))
  }else{
    prob_test <- data.frame(coord_Umap, object[[proba_col]])
    ggplot(prob_test, aes(UMAP_1,UMAP_2, color = prob_test[[proba_col]]))+geom_point()+theme_cowplot()+scale_color_gradientn(name = proba_col ,colors = c("lightgrey","blue"), limits = c(min(prob_test[[proba_col]]),max(prob_test[[proba_col]])))
  }
}
